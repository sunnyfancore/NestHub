...
type SearchEngine = 'baidu' | 'google' | 'bing' | 'sogou' | 'so360' | 'zhihu' | 'github'
      ...
      if (raw) {
        try {
          ...
          if (Date.now() - ts <= CACHE_TTL) {
            const authChanged = authStore.isAuthenticated && !data.canEdit
            const viewChanged = isPublicView.value !== !!cachedPublicView
            if (!authChanged && !viewChanged) {
...
function initSearchEngine() {
  ...
  if (saved && ['baidu', 'google', 'bing', 'sogou', 'so360', 'zhihu', 'github'].includes(saved)) {
...
async function togglePublicView() {
  isPublicView.value = !isPublicView.value
...
function resolveIcon(link: PortalLink) {
  ...
  try {
    ...
    return `https://favicon.png.pub/v1/${base64}`
  } catch {
    return ''
  }
}
...
function handleIconError(event: Event) {
  ...
  if (!span) return
  const link = span.closest('.portal-card') as HTMLElement | null
  const title = link?.querySelector('.portal-card__copy strong')?.textContent?.trim() || '?'
...
function setupSectionObserver() {
    ...
    (entries) => {
      for (const entry of entries) {
        if (entry.isIntersecting) {
          activeSectionId.value = entry.target.id.replace('section-', '').replace('child-', '')
        }
      }
    },
  ...
  document.querySelectorAll('.portal-section[id], .portal-subsection[id]').forEach((el) => {
  ...
  <div class="portal-page" :class="'theme-' + activeTheme" v-loading="loading">
    ...
    <aside class="portal-sidebar" :class="{ 'is-mobile-open': mobileSidebarOpen }">
      <div class="portal-sidebar__logo" :class="{ 'is-full': portal?.site.logoMode === 'full' }">
        ...
        <button class="portal-sidebar__close" @click="mobileSidebarOpen = false">
    ...
    <main class="portal-main">
      <header class="portal-searchbar">
        <button class="portal-hamburger" type="button" @click="mobileSidebarOpen = true">
        ...
        <div class="portal-searchbar__inner">
          <div class="portal-search-shell">
            ...
            />
            <button class="portal-search-submit" type="button" @click="runSearch">
        ...
        <div class="portal-searchbar__ops">
          <template v-if="authStore.isAuthenticated">
            <el-button
              text
              size="small"
              :type="isPublicView ? 'primary' : 'default'"
              @click="togglePublicView"
            >
              {{ isPublicView ? '公共视图' : '我的视图' }}
            </el-button>
            <el-button v-if="portal?.canEdit" text size="small" @click="openAdmin">
          ...
          <template v-else>
            <el-button text size="small" @click="openLoginDialog">
        ...
        >
          <div class="portal-section__head" @contextmenu.prevent="openCategoryContextMenu(category, $event)">
            ...
            <div v-if="portal?.canEdit" class="portal-section__actions">
              <el-button text size="small" @click="openCreateCategory(category.id)">
                <el-icon><Plus /></el-icon>
              </el-button>
              <el-button text size="small" @click="openCreateLink(category.id)">
            ...
            >
              <button class="portal-card__main" type="button">
                <span class="portal-card__icon">
                  ...
                  <img v-else :src="resolveIcon(link)" :alt="link.title" @error="handleIconError" />
                </span>
          ...
          >
            ...
            >
              <div class="portal-subsection__head" @contextmenu.prevent="openCategoryContextMenu(child, $event)">
                ...
                <div v-if="portal?.canEdit" class="portal-section__actions">
                  <el-button text size="small" @click="openCreateLink(child.id)">
                    <el-icon><Plus /></el-icon>
                  </el-button>
      ...
      <footer v-if="portal?.site.footerText" class="portal-footer">
        {{ portal.site.footerText }}
      </footer>
    </main>

    <div class="portal-right-rail">
      <el-button circle @click="toggleDarkMode" :title="isDark ? '切换亮色模式' : '切换暗色模式'">
        <i :class="['fa', isDark ? 'fa-sun-o' : 'fa-moon-o']"></i>
      </el-button>
      <template v-if="portal?.canEdit">
        <el-button circle @click="openCreateLink()">
          <el-icon><Plus /></el-icon>
        </el-button>
        <el-button circle @click="openAdmin">
          <el-icon><Setting /></el-icon>
        </el-button>
      ...
      <template v-else-if="!authStore.isAuthenticated">
        <el-button circle @click="openLoginDialog">
          <el-icon><Connection /></el-icon>
        </el-button>
      </template>
      <el-button circle @click="loadPortal(true)">
    ...
    <div class="portal-bottom-dock">
      ...
      <template v-else>
        <el-button round type="primary" @click="openLoginDialog">
          <el-icon><Connection /></el-icon>
          登录后管理
        </el-button>
      </template>
...
.portal-searchbar__ops :deep(.el-button) {
...
.theme-ocean .portal-content { gap: 28px; padding: 32px 32px 80px; }
...
.theme-glass .portal-sidebar {
  background: rgba(30, 20, 60, 0.85); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px);
...
.theme-glass .portal-content { gap: 24px; padding: 28px 28px 80px; }
...
.theme-glass .portal-section {
  ...
  border: 1px solid rgba(255,255,255,0.4); border-radius: 16px; padding: 24px 24px 26px;
...
.theme-glass .portal-card {
  ...
  border: 1px solid rgba(255,255,255,0.3); border-radius: 16px; padding: 16px 18px; gap: 14px;
  transition: all 0.25s ease;
}
.theme-glass .portal-card:hover {
  background: rgba(255,255,255,0.7); border-color: rgba(124, 77, 255, 0.3);
...
.theme-glass .portal-engine-menu { background: rgba(255,255,255,0.7); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); border: 1px solid rgba(255,255,255,0.4); border-radius: 16px; }
...
.theme-glass .portal-suggest { background: rgba(255,255,255,0.7); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); border: 1px solid rgba(255,255,255,0.4); border-radius: 16px; }
...
html.dark .theme-glass .portal-searchbar { background: rgba(15, 10, 40, 0.75); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); border-color: rgba(124, 77, 255, 0.15); }
html.dark .theme-glass .portal-search-shell { background: rgba(30, 20, 60, 0.6); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); border: 2px solid #7c4dff; border-radius: 20px; }
...
html.dark .theme-glass .portal-sidebar { background: rgba(10, 5, 30, 0.9); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); box-shadow: 6px 0 32px rgba(0,0,0,0.3); }
...
html.dark .theme-glass .portal-card { background: rgba(255,255,255,0.04); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); border: 1px solid rgba(255,255,255,0.06); border-radius: 16px; }
html.dark .theme-glass .portal-card:hover { background: rgba(255,255,255,0.1); border-color: rgba(124, 77, 255, 0.25); box-shadow: 0 8px 32px rgba(124, 77, 255, 0.15); }
...
html.dark .theme-glass .portal-engine-menu { background: rgba(20, 10, 50, 0.85); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); border: 1px solid rgba(124, 77, 255, 0.15); border-radius: 16px; box-shadow: 0 8px 24px rgba(0,0,0,0.5); }
...
html.dark .theme-glass .portal-suggest { background: rgba(20, 10, 50, 0.85); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); border: 1px solid rgba(124, 77, 255, 0.15); border-radius: 16px; box-shadow: 0 8px 24px rgba(0,0,0,0.5); }
...
.theme-neon .portal-content { gap: 20px; padding: 24px 24px 80px; }
...
.theme-paper .portal-content { gap: 28px; padding: 32px 28px 80px; }
